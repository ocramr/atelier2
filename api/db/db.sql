-- MySQL Script generated by MySQL Workbench
-- 02/06/17 14:00:40
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mydb` DEFAULT CHARACTER SET utf8 ;
USE `mydb` ;

-- -----------------------------------------------------
-- Table `mydb`.`user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`user` (
  `id` INT NOT NULL,
  `nom` VARCHAR(255) NULL,
  `prenom` VARCHAR(255) NULL,
  `email` VARCHAR(255) NULL,
  `username` VARCHAR(255) NULL,
  `password` VARCHAR(255) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`point`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`point` (
  `id` INT NOT NULL,
  `lng` VARCHAR(255) NULL,
  `lat` VARCHAR(255) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`indice`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`indice` (
  `id` INT NOT NULL,
  `valeur` VARCHAR(255) NULL,
  `id_point` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `id_point_idx` (`id_point` ASC),
  CONSTRAINT `id_point`
    FOREIGN KEY (`id_point`)
    REFERENCES `mydb`.`point` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`level`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`level` (
  `id` INT NOT NULL,
  `nbr_essais` VARCHAR(255) NULL,
  `distance` VARCHAR(255) NULL,
  `temps` VARCHAR(255) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`partie`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`partie` (
  `id` INT NOT NULL,
  `pseudo` VARCHAR(255) NULL,
  `token` VARCHAR(255) NULL,
  `score` VARCHAR(255) NULL,
  `etat` VARCHAR(255) NULL,
  `temps_partie` VARCHAR(255) NULL,
  `id_level` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `id_level_idx` (`id_level` ASC),
  CONSTRAINT `id_level`
    FOREIGN KEY (`id_level`)
    REFERENCES `mydb`.`level` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`point_partie`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`point_partie` (
  `id_point` INT NOT NULL,
  `id_partie` INT NULL,
  `is_final` TINYINT(1) NULL,
  PRIMARY KEY (`id_point`),
  INDEX `id_partie_idx` (`id_partie` ASC),
  CONSTRAINT `id_point`
    FOREIGN KEY (`id_point`)
    REFERENCES `mydb`.`point` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_partie`
    FOREIGN KEY (`id_partie`)
    REFERENCES `mydb`.`partie` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
